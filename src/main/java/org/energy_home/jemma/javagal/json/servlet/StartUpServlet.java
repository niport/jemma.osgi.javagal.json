...
public class StartUpServlet extends CommonServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			boolean start = getBooleanParameter(request, Resources.URI_PARAM_START);

			try {
				StartupAttributeInfo defaultStartUpAttributeInfo = parseJsonBody(request, StartupAttributeInfo.class);
				Status status = gatewayInterface.startGatewayDeviceSync(timeout, defaultStartUpAttributeInfo);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			long index = getLongParameter(request, Resources.URI_PARAM_INDEX);
