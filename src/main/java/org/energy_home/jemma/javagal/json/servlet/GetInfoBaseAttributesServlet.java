...
public class GetInfoBaseAttributesServlet extends CommonServlet {
	private static final long serialVersionUID = 1L;
	GatewayInterface gatewayInterface;
	Gson gson;

	public GetInfoBaseAttributesServlet(GatewayInterface _gatewayInterface) {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		long attributeId = this.getLongParameter(request, Resources.PARAMETER_ID);

		try {
			String result = gatewayInterface.getInfoBaseAttribute((short) attributeId);
